...
// ============================================================
// 🔥 GLOBALBAZAAR & GBPAY - SAFE PAYOUT SCRIPT
// ============================================================
...
function togglePayoutFields(type) {
    if (isPayoutLocked) {
        if (typeof showPopup === 'function') showPopup('error', '🔒 Locked!', 'Payout method is permanently locked.');
        return;
    }
...
function permanentLockPayout() {
    ...
    if (document.getElementById('payoutLockStatus')) document.getElementById('payoutLockStatus').style.display = 'block';
    if (document.getElementById('lockPayoutBtn')) document.getElementById('lockPayoutBtn').style.display = 'none';

    alert('🔒 Payout method permanently locked!');
}
...
function processWithdrawalRequest() {
    ...
    if (!isPayoutLocked) {
        alert('Please lock your payout method first.');
        return;
    }

    alert('Withdrawal request of ' + amount + ' processed successfully via ' + savedPayoutData?.type);
}

// Check status safely without blocking page
function checkPayoutStatus() {
    try {
        ...
        if (locked === 'true') {
            ...
            if (data) {
                savedPayoutData = data;
                if (data.type === 'bank' && document.getElementById('sellerBankAccountNo')) {
                    const bankRadio = document.querySelector('input[name="sellerPayoutMethod"][value="bank"]');
                    if (bankRadio) bankRadio.checked = true;
                    document.getElementById('sellerBankHolderName').value = data.holder || '';
                    document.getElementById('sellerBankAccountNo').value = data.account || '';
                    document.getElementById('sellerBankIfsc').value = data.ifsc || '';
                    document.getElementById('sellerBankName').value = data.bankName || '';
                    togglePayoutFields('bank');
                } else if (document.getElementById('withdrawCryptoAddress')) {
                    ...
                    document.getElementById('sellerCryptoCoinType').value = data.coinType || 'usdt';
                    document.getElementById('withdrawCryptoAddress').value = data.wallet || '';
